...
#
# 2011-03-10 (ADH): Numerous changes. Whatever Instiki's trying to do, the
#                   results often seem broken - it's possible to get a 500
#                   error in a diff view, for example, because UTF 8 byte
#                   sequences are getting split mid-way through. Took out as
#                   much custom code as possible and tried to lean on the
#                   Rails multibyte support. We're using Rails 2.3.11 and
#                   Ruby 1.8; this combination seems to work properly.
#
class String
	...
	def unescapeHTML
  	self.gsub(/&(.*?);/) do
    	...
    	when /\A#0*(\d+)\z/n       then
      	if Integer($1) < 256
        	# 2011-03-10 (ADH): Wrong; in Ruby 1.8, there is no encoding support
        	#                   so this returns an ISO 8859-1 single byte value.
        	#                   In Ruby 1.9, Encoding::UTF8 would need to be
        	#                   specified, but isn't.
        	#Integer($1).chr
        	#
        	# Could do this:
        	#
        	#   Iconv.iconv('utf-8', 'iso-8859-1', Integer($1).chr)
        	#
        	# ...but this is probably faster and copies the hex handling code
        	# below, which after experiments in "irb", does seem to work well:
        	[Integer($1)].pack("U")
      	else
